...
import { Upload, Plus, X, Receipt, Users, DollarSign, Crown, Lock, Share } from "lucide-react"
...
${result.items
  .map(
    (item: any) =>
      `• ${item.name} ${item.sharedWith > 1 ? `(dibagi ${item.sharedWith})` : ""}: Rp ${item.price.toLocaleString()}`,
  )
  .join("\n")}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-100/50 relative overflow-hidden">
      ...
      <div className="ml-72 p-6 relative z-10">
        ...
        <div className="flex items-center space-x-4 mb-8">
          <h1 className="text-4xl font-bold text-slate-800 font-display">SplitBill</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="card-modern border-0 relative overflow-hidden">
            ...
            <CardHeader className="relative z-10 opacity-50">
              <CardTitle className="text-sm font-medium text-slate-600 font-body">Total Bills</CardTitle>
            ...
            <CardContent className="relative z-10 opacity-50">
              <div className="text-3xl font-bold text-slate-800 mb-1 font-display">12</div>
          ...
          <Card className="card-modern border-0 relative overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-slate-100/80 to-slate-200/80 backdrop-blur-sm z-5"></div>
            <CardHeader className="relative z-10 opacity-50">
              <CardTitle className="text-sm font-medium text-slate-600 font-body">Total Amount</CardTitle>
            ...
            <CardContent className="relative z-10 opacity-50">
              <div className="text-3xl font-bold text-slate-800 mb-1 font-display">Rp 2.450.000</div>
              <p className="text-sm text-slate-500 font-body">Total semua bills</p>
            </CardContent>
            <div className="absolute inset-0 flex items-center justify-center z-20">
              <Button
                onClick={() => setShowPremiumModal(true)}
          ...
          <Card className="card-modern border-0 relative overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-slate-100/80 to-slate-200/80 backdrop-blur-sm z-5"></div>
            <CardHeader className="relative z-10 opacity-50">
              <CardTitle className="text-sm font-medium text-slate-600 font-body">Completed</CardTitle>
            ...
            <CardContent className="relative z-10 opacity-50">
              <div className="text-3xl font-bold text-slate-800 mb-1 font-display">8</div>
                ...
                {stepNumber < 4 && (
                  <div
                    className={`w-16 h-1 mx-3 transition-all duration-300 ${
                      step > stepNumber ? "bg-gradient-to-r from-primary-500 to-secondary-500" : "bg-slate-200"
                    }`}
                  />
                )}
              </div>
          ...
          <Card className="card-modern border-0 max-w-2xl mx-auto">
            ...
            <CardContent>
              <div className="border-2 border-dashed border-slate-300 rounded-2xl p-16 text-center hover:border-primary-400 transition-colors">
                ...
                <p className="text-slate-600 mb-8 text-lg font-body">Drag & drop atau klik untuk upload struk</p>
                ...
                {uploadedFile && (
                  <div className="mt-10 p-8 bg-slate-50 rounded-2xl">
                    ...
                    <p className="text-slate-700 font-medium font-body text-lg">Memproses: {uploadedFile.name}</p>
        ...
        {step === 3 && (
          <Card className="card-modern border-0 max-w-4xl mx-auto">
            ...
            <CardContent className="space-y-8">
              {menuItems.map((item) => (
                <div key={item.id} className="border border-slate-200 rounded-2xl p-6 glass-effect">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="font-semibold text-slate-800 text-xl font-display">{item.name}</h3>
              ...
              <div className="border border-accent-300 rounded-2xl p-6 bg-gradient-to-r from-accent-50 to-yellow-50">
                <h3 className="font-semibold text-slate-800 mb-6 text-xl font-display">Biaya Tambahan</h3>
                <div className="grid grid-cols-3 gap-6">
                  <div>
                    <Label className="text-slate-700 font-medium font-body">Pajak</Label>
                    <Input
                      type="number"
                  ...
                  <div>
                    <Label className="text-slate-700 font-medium font-body">Service</Label>
                    <Input
                      type="number"
                  ...
                  <div>
                    <Label className="text-slate-700 font-medium font-body">Tip</Label>
                    <Input
                      type="number"
                ...
                <CardContent>
                  <div className="space-y-4">
                    {result.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between text-sm py-3 border-b border-slate-100">
                    ...
                    <div className="flex justify-between text-sm py-3 border-b border-slate-100">
                      <span className="text-slate-600 font-body">Biaya tambahan</span>
                    ...
                    <div className="flex justify-between font-bold text-xl pt-4 border-t-2 border-accent-200">
                      ...
                      <span className="text-accent-600 font-display">Rp {result.total.toLocaleString()}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <Card className="card-modern border-0 mt-8 relative overflow-hidden">
          ...
          <CardHeader className="relative z-10 opacity-50">
            ...
            <CardDescription className="font-body">Lihat semua riwayat split bill yang pernah dibuat</CardDescription>
          </CardHeader>
          <CardContent className="relative z-10 opacity-50">
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-slate-800 font-display">Resto Padang Sederhana</h3>
                      <p className="text-sm text-slate-500 font-body">20 Jan 2024 • 4 orang</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-slate-800 font-display">Rp 320.000</p>
                    <Badge className="bg-emerald-100 text-emerald-700 border-emerald-200 text-xs">Selesai</Badge>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center z-20">
            <div className="text-center p-8">
              ...
              <h3 className="text-xl font-bold text-slate-800 font-display mb-2">Premium Feature</h3>
